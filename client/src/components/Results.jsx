  ...
  useEffect(() => {
    async function fetchData() {
      const response = await fetch(
        `https://us-real-estate.p.rapidapi.com/for-sale?offset=0&limit=42&state_code=GA&city=Atlanta&sort=newest&beds_min=3&baths_min=2&property_type=single_family&home_size_min=1200&stories=single`,
        {
          headers: {
            "X-RapidAPI-Key":
          ...
          {data ? (
            <div>
              <img
                className="homeimage"
                alt="homeimage"
                src={data.results[3].primary_photo.href || { Home }}
              ></img>
            </div>
          ) : (
            <p>Loading...</p>
